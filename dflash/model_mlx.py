...
class DFlashConfig:
    ...
    # Scale factor applied to attention logits; defaults to head_dim^-0.5 if None
...
class DFlashAttention(nn.Module):
    def __init__(self, config: DFlashConfig):
        ...
        # Allow overriding the attention scale via config (useful for experimentation)
        self.scale = config.attn_scale_override if config.attn_scale_override is not None else config.head_dim ** -0.5
